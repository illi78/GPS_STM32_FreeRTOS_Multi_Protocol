...
#define RxBuf_SIZE 249			// 85
#define MainBuf_SIZE 500		// 170
...
volatile int isDataAvailable = 0;

volatile int isOK = 0;
volatile uint8_t num_OK = 0;
 ...
 * secured
 */
int copynUpto (char *string, char *buffertocopyinto, uint32_t Timeout, uint32_t buffsize)
{
	int so_far = 0;
	int len = strlen (string);
	int indx = 0;
...
again:
	...
	while (MainBuf[Tail] != string[so_far])
	{
		if (indx >= buffsize - 1) return 0;  // prevent overflow
		buffertocopyinto[indx++] = MainBuf[Tail];
	...
	while (MainBuf[Tail] == string[so_far])
	{
		if (indx >= buffsize - 1) return 0;
		buffertocopyinto[indx++] = MainBuf[Tail++];
		if (Tail == MainBuf_SIZE) Tail = 0;
		so_far++;
		if (so_far == len) {
			buffertocopyinto[indx] = '\0';  // null-terminate
			return 1;
		}
	}
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
	...
	/* Let's say we want to check for the keyword "<CR><LF>" within our incoming DATA */
...
*/
	/* Let's say we want to check for the keyword "<CR><LF>" within our incoming DATA */
	/* The data can be more than 1 sentence followed by each other (like GSV response) */
	for (int i = 0 ; i < Size - 1 ; i++) {
	    if (MainBuf[i] == '\r' && MainBuf[i + 1] == '\n')
	    {
	        num_OK++;

			// mark if ISR released a higher priority task than current task (false: currently didn't)
	        BaseType_t xHigherPriorityTaskWoken = pdFALSE;
	        // notify CommunicationTask_Handler it can start immediately and has higher priority
//	        vTaskNotifyGiveFromISR(SensorsTask_Handler, &xHigherPriorityTaskWoken);				// CommunicationTask_Handler

	        xTaskNotifyFromISR(
	        	SensorsTask_Handler,
	            EVT_GPS_SENTENCE_READY,
	            eSetBits,
	            &xHigherPriorityTaskWoken);

	        // if == pdTRUE, higher priority task is READY - Start immediately
